...
void updateSystem() {
    ...
    for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
        ...
        nrows +=  3 * numberOfNeighbors;   // WHAT TO PUT HERE ??????? How to update the number of rows ?
    }
    for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
    ...
    for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
        ...
        for( std::map< unsigned int , double >::const_iterator it = edgeAndVertexWeights.get_weight_of_adjacent_edges_it_begin(v) ;
            ...
            arapLinearSystem.A(equationIndex,3 * v) = -1.0; 
            arapLinearSystem.A(equationIndex,vNeighbor * 3) = 1.0; 
            
            equationIndex++;
            arapLinearSystem.A(equationIndex,3 * v + 1) = -1.0; 
            arapLinearSystem.A(equationIndex,3 * vNeighbor  + 1) = 1.0; 
            equationIndex++;

            arapLinearSystem.A(equationIndex,3 * v + 2) = -1.0; 
            arapLinearSystem.A(equationIndex,3 * vNeighbor + 2) = 1.0; 
            equationIndex++;
    ...
    for( unsigned int arapIteration = 0 ; arapIteration < maxIterationsForArap ; ++arapIteration ) {
        ...
        for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
            ...
            for( std::map< unsigned int , double >::const_iterator it = edgeAndVertexWeights.get_weight_of_adjacent_edges_it_begin(v) ;
                ...
                for(int i = 0;i< 3;i++){
                    tensorMatrix(i,0) += initialEdge[i] * rotatedEdge[0];
                    tensorMatrix(i,1) += initialEdge[i] * rotatedEdge[1];
                    tensorMatrix(i,2) += initialEdge[i] * rotatedEdge[2];
